...
describe('NaturalForm', () => {
  ...
  it('should show errors on submit', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Este campo deve ter no mínimo 11 caracteres.')).toBeInTheDocument();
      expect(screen.getByText('Você deve ter no mínimo 18 anos.')).toBeInTheDocument();
      expect(screen.getByText('Documento inválido.')).toBeInTheDocument();
      expect(screen.getByText('Seu faturamento mensal não é suficiente.')).toBeInTheDocument();
  ...
  it('should call consultPerson correctly', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Nome Completo'), { target: { value: 'João da Silva' } });
    ...
    fireEvent.change(screen.getByLabelText('Renda Mensal'), { target: { value: '3000' } });
  ...
  describe('input validations', () => {
    it('should show an error if name has less than 8 characters', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Este campo deve ter no mínmo 11 caracteres.')).toBeInTheDocument();
      });
    });
    ...
    it('should render an error when age under 18', async () => {
      ...
      fireEvent.change(screen.getByLabelText('Idade'), { target: { value: '17' } });
    ...
    it('should render an error when monthly income is 0', async () => {
      ...
      fireEvent.change(screen.getByLabelText('Renda Mensal'), { target: { value: '0' } });
  ...
  describe('actions', () => {
    it('should not call consultPerson if has errors', async () => {
      ...
      fireEvent.change(screen.getByLabelText('Nome Completo'), { target: { value: 'João' } }); // Nome inválido
      fireEvent.submit(screen.getByRole('form'));
